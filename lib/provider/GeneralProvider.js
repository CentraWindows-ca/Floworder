...
export const GeneralProvider = ({ children, permissions, rawAuth }) => {
  ...
  const { data: facilitySystemCodeList } = useDataInit(
    WMCommonQuery.initGetFacilitySystemCode,
  );
  ...
  useEffect(() => {
    ...
    if (facilitySystemCodeList) {
      setDictionary((prev) => ({
        ...prev,
        facilitySystemCodeList,
      }));
    }
    if (uiItemLabels) {
      setDictionary((prev) => ({
