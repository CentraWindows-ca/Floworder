...
import syncFFApi from "./syncFFApi";
...
const uploadFileAsync = async (body) => {
  ...
  // sync back to FF
  await syncFFApi.updateFiles2FFAsync({ masterId: body.masterId });
...
const deleteUploadFileByIdAsync = async (params) => {
  ...
  // sync back to FF
  await syncFFApi.updateFiles2FFAsync({ masterId: params.masterId });
  return data;
};
...
const uploadImageAsync = async (body) => {
  ...
  // sync back to FF
  await syncFFApi.updateImages2FFAsync({ masterId: body.masterId });
...
const deleteUploadImageByIdAsync = async (params) => {
  ...
  // sync back to FF
  await syncFFApi.updateImages2FFAsync({ masterId: params.masterId });
...
const sync_AB_WindowMakerByWorkOrderAsync = async (params, body) => {
  ...
  // sync back to FF
  await syncFFApi.sync_AB_Order_FF_Async({
...
const sync_BC_WindowMakerByWorkOrderAsync = async (params, body) => {
  ...
  // sync back to FF
  await syncFFApi.sync_BC_Order_FF_Async({
    workOrderNo: body.workOrderNo,
    masterId: data?.masterId,
  });
...
const batchUpdateTableAsync = async (params, body) => {
  ...
  switch (table) {
    case "ProdWindowItems":
      await syncFFApi.updateWinItems2FFAsync({ masterId });
      break;
    case "ProdDoorItems":
      await syncFFApi.updateDoorItems2FFAsync({ masterId });
      break;
    default:
      break;
  }
...
const updateWorkOrderHeaderWithPrefixAsync = async (
  ...
  // sync back to FF
  await syncFFApi.updateHeaderData2FFAsync(
    {
      masterId,
      workOrderNo,
    },
