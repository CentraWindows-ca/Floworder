...
const updateWorkOrderStatus = async (
  ...
  if (
    ...
    oldStatus === 'Scheduled Work Order' && newStatus === 'Confirmed Reservations'
  ) {
    api = `UpdateWorkOrderStatus?${utils.paramToString({}, true)}`;
  }
...
const updateProductionsReturnTrip = async (params, body, ref, refRt) => {
  ...
  const data = await fetcher(api, ROOT, { method: "POST", body: fields });

  // sync back to FF
  await Prod2FFApi.updateReturnTrip2FFAsync(ref);

  // sync back to history
  await ProdHistoryApi.insertWorkOrderHistoryWhenUpdateReturnTripAsync({
    masterId: ref.m_MasterId,
    fields,
  });
...
const addProductionsReturnTrip = async (params, body, ref, refRt) => {
  ...
  // sync back to history
  await ProdHistoryApi.insertWorkOrderHistoryWhenAddReturnTrip({
    masterId: ref.m_MasterId,
    fields: { ...fields, id: data?.recordId },
  });
...
const hardDeleteProductionsReturnTrip = async (params, body, ref) => {
  ...
  // sync back to history
  await ProdHistoryApi.insertWorkOrderHistoryWhenDeleteReturnTripAsync({
    masterId: ref?.m_MasterId,
    fields,
  });
