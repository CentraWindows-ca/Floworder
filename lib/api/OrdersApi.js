...
const getAddsOnGroupByMasterId = async (params, body) => {
  // const api = `GetAddOnsGroupByMasterId?${utils.paramToString(params, true)}`;
  const api = `GetAddOnsGroupFacilitySplitByMasterId?${utils.paramToString(params, true)}`;
...
const queryWorkOrderHeaderWithPrefixAsync = async (params, body) => {
  ...
  const api = `QueryWorkOrderHeaderWithFacilitySplitAsync?${utils.paramToString(params, true)}`;
...
const initQueryWorkOrderHeaderWithPrefixAsync = (params, body) => [
  ...
  `QueryWorkOrderHeaderWithFacilitySplitAsync`,
  ROOT,
  {
    method: "POST",
    body,
...
const updateWorkOrderHeaderWithPrefixAsync = async (
  ...
  // const api = `UpdateWorkOrderHeaderWithFacilitySplitAsync?${utils.paramToString({ ...params }, true)}`;
  const api = `UpdateOMHeaderData?${utils.paramToString({ ...params }, true)}`;

  const parsedObj = converToString(fields);
  if (_.isEmpty(parsedObj)) return null;

  // const subOrderIds = getSubOrderIds(refStructure);
...
const updateWorkOrderStatus = async (
  ...
  let api = `UpdateWorkOrderStatus_Facility?${utils.paramToString({}, true)}`;
...
const getSiteLockoutByMasterId = async (params, body) => {
  try {
    const api = `GetLinkedSiteLockoutByMasterId?${utils.paramToString(params, true)}`;
    const res = await fetcher(api, ROOT);
...
const getSubOrderIds = (refStructure) => {
  const subOrderIds = {};
  _.keys(refStructure)?.map((k) => {
    if (k !== "m") {
      subOrderIds[k] = refStructure?.[k]?.[`${k}_Id`];
    }
  });
