...
import { ALL_SUBORDER_TYPES } from ".";
...
export const uiWoFieldEditGroupMapping = {
  ...
  options: {
    ...
    d_MultipointFlag: false,
    d_ShapesRequires: false,
  },
  schedule: {
...
export const applyField = (arr) => {
  return arr
    ...
    ?.map((a) => {
      ...
      if (!_v.title) {
        _v.title =
          getFieldObj({ fieldCode: a.id })?.title ||
          a.id ||
          getFieldObj({ fieldCode: a.key })?.title ||
          a.key;
      }
      return _v;
    });
...
/*
  ...
  fieldCode -> w_Status
*/
export const parseFieldsByfieldCode = (fieldCode, data) => {
  // fieldCode can be w_Aaaa, data will be like [{field: 'w_1_Aaaa'}, {field: 'w_2_Aaa'}]
};
export const parseFieldsByDbColumn = (dbColumn, data) => {
  //
};
