...
export const getOrderKind = (data) => {
  ...
  // if (data?.m_DoorStatus) {
  //   isDoor = true;
  // }


  isWindow = _.some(_.keys(data), (k) => k.startsWith("w_"))
  isDoor = _.some(_.keys(data), (k) => k.startsWith("d_"))
...
function formatDatetimeForMorganLegacy(input) {
...
const formatCurrencyStringToNumber = (input) => {
  // Pass through null/undefined directly
  if (input === null || input === undefined) return input;

  // If it's already a number, just return it
  if (typeof input === "number") {
    return Number.isNaN(input) ? input : input;
  }
  ...
  // Handle leading negative sign: -$1,234.56
  if (str[0] === "-") {
    isNegative = true;
  ...
  // (currency symbols, spaces, etc. will be removed)
  str = str.replace(/[^0-9.,]/g, "");

  // For now we assume decimal separator is ".", comma is thousand separator.
  // So we simply remove all commas.
  str = str.replace(/,/g, "");
  ...
  // If parsing fails, return original input instead of breaking the app
  if (Number.isNaN(num)) return input;

  return isNegative ? -num : num;
};

const formatDateForMorganLegacy = (str) => {
  if (!str) return null;
...
export const utils = {
  ...
  formatDate,
  formatDateForMorganLegacy,
  formatCurrencyStringToNumber,
  formatDatetimeForMorganLegacy,
